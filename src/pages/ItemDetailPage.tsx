...
import { CATEGORY_EMOJIS } from "@/lib/marketplace";
...
const ItemDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      ...
      if (error) {
        console.error("[item-detail] item fetch failed", error);
      }
  ...
  const handleRequest = async () => {
    ...
    try {
      ...
      console.error("[item-detail] request failed", err);
  ...
  const handleSendMessage = async () => {
    if (!user) {
      toast.error("Please sign in to message this Bonitar");
      return;
    }

    if (!item) return;

    if (user.id === item.donor_id) {
    ...
    try {
      const conversation = await createOrGetConversation(item.donor_id, item.id);
      navigate(`/chat?conversation=${conversation.id}`);
    } catch (error) {
      console.error("[item-detail] failed to open chat", error);
  ...
  const isOwner = !!item && user?.id === item.donor_id;
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="space-y-3">
              <div className="relative aspect-square bg-accent rounded-xl overflow-hidden flex items-center justify-center">
                {hasImages ? (
                  ...
                  <span className="text-7xl">{CATEGORY_EMOJIS[item.category] || "📦"}</span>
                )}
              </div>
            ...
            <div className="space-y-5">
              <div>
                ...
                <span
                  className={`ml-2 text-xs font-body font-medium px-2.5 py-0.5 rounded-full ${
                    item.status === "available" ? "bg-green-100 text-green-700" : "bg-muted text-muted-foreground"
                  }`}
                >
              ...
              <div className="flex items-center gap-4 text-sm text-muted-foreground font-body flex-wrap">
                {item.pickup_location && (
                  <span className="flex items-center gap-1">
                    <MapPin className="h-4 w-4" /> {item.pickup_location}
                  </span>
                )}
                {(item.state || item.area) && <span>{[item.area, item.state].filter(Boolean).join(", ")}</span>}
                <span className="flex items-center gap-1">
              ...
              {item.description && (
                <div>
                  ...
                  <p className="font-body text-muted-foreground text-sm leading-relaxed whitespace-pre-wrap">
                    {item.description}
                  </p>
                </div>
              )}
              ...
              {item.category === "Medication" && (
                <div className="p-3 rounded-xl bg-secondary/10 border border-secondary/20 space-y-1">
                  <p className="font-body text-xs text-foreground">
                    <strong>Medication Details</strong>
                  </p>
                  {item.medicine_name && (
                    <p className="font-body text-xs text-muted-foreground">Name: {item.medicine_name}</p>
                  )}
                  {item.medicine_usage && (
                    <p className="font-body text-xs text-muted-foreground">Used for: {item.medicine_usage}</p>
                  )}
                  {item.expiration_date && (
                    <p className="font-body text-xs text-muted-foreground">Expires: {item.expiration_date}</p>
                  )}
                </div>
              )}
